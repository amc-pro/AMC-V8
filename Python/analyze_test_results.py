...
print(f"==========================================================================")
print(f"               ANALYSE DES TRADES EXECUTÉS DANS SHADOW                    ")
print(f"==========================================================================")
...
if trades:
    ...
    print(f"  * Espérance E[R]   : {expectancy:+.2f} R / trade")
    ...
    for g, g_trades in sorted(by_grade.items(), key=lambda x: len(x[1]), reverse=True):
        g_wins = [t for t in g_trades if t["is_win"]]
        g_r = sum(t["r_multiple"] for t in g_trades)
        g_wr = (len(g_wins) / len(g_trades)) * 100
        print(f"  * {g:15s} : {len(g_trades):2d} trades | WR {g_wr:5.1f}% | Net R: {g_r:+6.2f} R")
    ...
    print(f"\n--- Liste Complète des Derniers Trades ---")
