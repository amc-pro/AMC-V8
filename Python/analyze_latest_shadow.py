...
if os.path.exists(outcomes_path):
    print(f"Fichier trouvé, taille: {os.path.getsize(outcomes_path)} bytes, mtime: {os.path.getmtime(outcomes_path)}")
    try:
        df_outcomes = pd.read_csv(outcomes_path, sep=';', on_bad_lines='skip')
        print(f"Total des trades émis: {len(df_outcomes)}")
        print(df_outcomes.to_string())
    except Exception as e:
        print("Erreur lecture outcomes:", e)
else:
    print("Fichier outcomes introuvable.")

print("\n" + "=" * 110)
print("2. ANALYSE DU JOURNAL SNIPER SUR LA SESSION DU 24/08 (16H00 A 18H30)")
print("=" * 110)

journal_path = 'shadow/AuctionMarketCorePro_journal_sniper.csv'
if os.path.exists(journal_path):
    print(f"Journal trouvé, taille: {os.path.getsize(journal_path)} bytes")
    try:
